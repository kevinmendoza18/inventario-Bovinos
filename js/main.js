...
function guardarInventario() {
  const hembras = getRows('tabla-hembras');
  const machos = getRows('tabla-machos');
  const datos = {
...
function cargarInventario() {
  ...
  if (!datos) {
    comprasTemp.hembras = [0,0,0,0,0,0];
    comprasTemp.machos = [0,0,0,0,0,0];
    ventasTemp.hembras = [0,0,0,0,0,0];
    ventasTemp.machos = [0,0,0,0,0,0];
    actualizarTotalesGenerales();
    return;
  }
  let hembras = getRows('tabla-hembras');
  let machos = getRows('tabla-machos');
